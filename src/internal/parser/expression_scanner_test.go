...
func Test_IfCanScan_SelectPlayer_ExpressionAndGetCorrectTypeAndTokens(t *testing.T) {
	...
	err := scanner.scanRawExpression("SELECT key, elo FROM Players;")
	...
	if result[0].Value[0] != "key" {
		t.Errorf("Invalid logical token")
	}
	if result[0].Value[1] != "elo" {
		t.Errorf("Invalid logical token")
	}
}

func Test_IfCanScan_SelectPlayersWithAllComponentsByHand_ExpressionAndGetCorrectTypeAndTokens(t *testing.T) {
	scanner := newExpressionScannerMock()

	err := scanner.scanRawExpression("SELECT key, elo, events, matches FROM Players;")
	...
	if result[0].Value[2] != "events" {
		t.Errorf("Invalid logical token")
	}
	if result[0].Value[3] != "matches" {
		t.Errorf("Invalid logical token")
	}
}

func Test_IfCanScan_SelectPlayersWithAllComponentsAuto_ExpressionAndGetCorrectTypeAndTokens(t *testing.T) {
	scanner := newExpressionScannerMock()

	err := scanner.scanRawExpression("SELECT * FROM Players;")
...
func Test_IfCanScan_SelectPlayer_ExpressionAndGetCorrectTypeAndTokens_OrderByEloAsc(t *testing.T) {
	...
	err := scanner.scanRawExpression("SELECT * FROM Players ORDER BY Elo ASC;")
...
func Test_IfCanScan_SelectPlayer_ExpressionAndGetCorrectTypeAndTokens_OrderByKeyDesc(t *testing.T) {
	...
	err := scanner.scanRawExpression("SELECT * FROM Players ORDER BY Key DESC;")
